...
class DaumArticleRequestCrawler():
    def __init__(self,
                 logger: logging.Logger,
                 db_helper: ArticleSqliteHelper,
                 start_year: int,
    ...
    def __get_article(self, news_link: str):
        ...
        if response.status_code != 200:
            self._logger.error(f'{news_link} load article error occurred')
            return
    ...
    def __loop_day(self, date_str: str):
        ...
        while True:
            try:
                ...
                if response.status_code != 200:
                    self._logger.error(f'date: {date_str}\tpage: {page} load page error occurred')
                    continue
